    ...
    <div tabindex="0" class="story-item" data-storyid="${id}">
      ...
      <div class="story-item__body">
        <div class="story-item__main">
         ...
         <div class="story-item__location" style="display: flex; flex-direction: column; gap: 8px; margin-top: 12px;">
  ${
    location?.latitude && location?.longitude
      ? `
        <div class="story-item__map">
          <iframe
            width="100%"
            height="100"
            ...
            loading="lazy"
            allowfullscreen
            src="https://www.google.com/maps?q=${location.latitude},${
          location.longitude
        }&hl=id&z=15&output=embed">
          </iframe>
        </div>
        <div class="story-item__place" style="display: flex; align-items: center; gap: 6px; color: white;">
          <i class="fas fa-map"></i>
          <span>${location.placeName ? location.placeName : ""}</span>
        </div>
      `
      : `
        <div class="story-item__place" style="display: flex; align-items: center; gap: 6px; color: #555;">
          <i class="fas fa-map"></i>
...
export function generateStoryDetailTemplate({
  ...
  const imagesHtml = Array.isArray(photoUrl)
    ? photoUrl.reduce(
        (accumulator, url) =>
          accumulator.concat(
            generateStoryDetailImageTemplate(url, title ?? "")
          ),
        ""
      )
